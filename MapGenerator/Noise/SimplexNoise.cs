    ...
    public class SimplexNoise
    {
        ...
        public int Seed
        {
            ...
            set
            {
                if (value == 0)
                {
                    m_Perm = new byte[m_PermOriginal.Length];
                    m_PermOriginal.CopyTo(m_Perm, 0);
                }
                else
                {
                    m_Perm = new byte[512];
                    var random = new Random(value);
                    random.NextBytes(m_Perm);
                }
        ...
        private readonly byte[] m_PermOriginal =
        {
        ...
        private readonly byte[][] m_Simplex =
        {
            new byte[] {0,1,2,3},
            new byte[] {0,1,3,2},
            new byte[] {0,0,0,0},
        ...
        public float Calc1D(int x, float scale = 1)
        {
            return Generate(x * scale);
        }

        public float Calc2D(int x, int y, float scale = 1)
        {
            return Generate(x * scale, y * scale);
        }

        public float Calc3D(int x, int y, int z, float scale = 1)
        {
            return Generate(x * scale, y * scale, z * scale);
        }

        public float Calc4D(int x, int y, int z, int w, float scale = 1)
        {
            return Generate(x * scale, y * scale, z * scale, w * scale);
        }
        ...
        private float Generate(float x)
        {
            ...
            var t0 = 1.0f - x0 * x0;
            t0 *= t0;
            var n0 = t0 * t0 * Grad(m_Perm[i0 & 0xff], x0);

            var t1 = 1.0f - x1 * x1;
            t1 *= t1;
            var n1 = t1 * t1 * Grad(m_Perm[i1 & 0xff], x1);
        ...
        private float Generate(float x, float y, float z, float w)
        {
            ...
            {
                t0 *= t0;
                n0 = t0 * t0 * Grad(m_Perm[ii + m_Perm[jj + m_Perm[kk + m_Perm[ll]]]], x0, y0, z0, w0);
            }
            ...
            {
                t1 *= t1;
                n1 = t1 * t1 * Grad(m_Perm[ii + i1 + m_Perm[jj + j1 + m_Perm[kk + k1 + m_Perm[ll + l1]]]], x1, y1, z1, w1);
            }
            ...
            {
                t2 *= t2;
                n2 = t2 * t2 * Grad(m_Perm[ii + i2 + m_Perm[jj + j2 + m_Perm[kk + k2 + m_Perm[ll + l2]]]], x2, y2, z2, w2);
            }
            ...
            {
                t3 *= t3;
                n3 = t3 * t3 * Grad(m_Perm[ii + i3 + m_Perm[jj + j3 + m_Perm[kk + k3 + m_Perm[ll + l3]]]], x3, y3, z3, w3);
            }
            ...
            {
                t4 *= t4;
                n4 = t4 * t4 * Grad(m_Perm[ii + 1 + m_Perm[jj + 1 + m_Perm[kk + 1 + m_Perm[ll + 1]]]], x4, y4, z4, w4);
            }
